...
import { renderTmj, appendMapRules, mapDataAsmSection, mapSnippet, mapSymbolBase } from './tilemapScaffold';
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('cooper.newTilemap', () => newTilemap()),
...
/**
 * New Tilemap (D-082): the maps counterpart of Add Sprite. The SDK's map
 * pipeline is Tiled-based; the cliff is the WIRING. Generates a ready-to-paint
 * .tmj bound to a project tileset PNG, wires the Makefile (gfx -m + tmx2snes
 * rules) and the data.asm bridge, and hands back the mapLoad C. Paint tiles,
 * collision (`attribute`) and spawns (Entities layer) in Tiled; Build converts.
 */
async function newTilemap(): Promise<void> {
    const projectDir = await resolveProjectDir();
    if (!projectDir) {
        failWithDownload('sdk', 'open an OpenSNES project first, then create a tilemap in it.');
        return;
    }
    // the tileset = an indexed PNG in the project
    const pngs = await vscode.workspace.findFiles(new vscode.RelativePattern(projectDir, '**/*.png'), '**/node_modules/**', 50);
    if (pngs.length === 0) {
        const go = await vscode.window.showInformationMessage(
            'A tilemap needs a tileset PNG (8\u00d78 tiles, \u226416 colours). Create one first?', 'New Sprite\u2026');
        if (go) {
            await newSprite();
        }
        return;
    }
    const tsPick = pngs.length === 1
        ? { fsPath: pngs[0].fsPath }
        : await vscode.window.showQuickPick(pngs.map((u) => ({ label: path.relative(projectDir, u.fsPath), fsPath: u.fsPath })), { placeHolder: 'Tileset PNG (its 8\u00d78 tiles become the map tiles)' });
    if (!tsPick) {
        return;
    }
    let img;
    try {
        img = readIndexedPng(fs.readFileSync(tsPick.fsPath));
    } catch (e) {
        fail(`${(e as Error).message}`);
        return;
    }
    if (img.width % 8 !== 0 || img.height % 8 !== 0) {
        fail(`the tileset must be a grid of 8\u00d78 tiles \u2014 this PNG is ${img.width}\u00d7${img.height}.`);
        return;
    }
    const name = await vscode.window.showInputBox({
        prompt: 'Map name', value: 'level1',
        validateInput: (v) => (/^[\w-]+$/.test(v.trim()) ? undefined : 'letters, digits, _ - only'),
    });
    if (!name) {
        return;
    ...
    const sizePick = await vscode.window.showQuickPick(
        [{ label: '64\u00d732 tiles (2 screens wide)', w: 64, h: 32 }, { label: '32\u00d732 tiles (1 screen)', w: 32, h: 32 }, { label: '128\u00d732 tiles (4 screens wide)', w: 128, h: 32 }],
        { placeHolder: 'Map size (8\u00d78 tiles \u2014 the engine scrolls it for you)' });
    if (!sizePick) {
        return;
    }
    const base = mapSymbolBase(name.trim());
    const tmjRel = `res/${name.trim()}.tmj`;
    const tsRel = path.relative(projectDir, tsPick.fsPath).split(path.sep).join('/');
    try {
        fs.mkdirSync(path.join(projectDir, 'res'), { recursive: true });
        fs.writeFileSync(path.join(projectDir, tmjRel), renderTmj({
            widthTiles: sizePick.w, heightTiles: sizePick.h,
            tilesetImage: path.basename(tsPick.fsPath), imageWidth: img.width, imageHeight: img.height,
        }));
        const mkPath = path.join(projectDir, 'Makefile');
        let mk = fs.readFileSync(mkPath, 'utf8');
        mk = ensureModules(mk, ['console', 'background', 'dma', 'input', 'map']);
        mk = ensureAsmSrc(mk, 'data.asm');
        mk = appendMapRules(mk, tmjRel, tsRel);
        fs.writeFileSync(mkPath, mk);
        const dataPath = path.join(projectDir, 'data.asm');
        const existing = fs.existsSync(dataPath) ? fs.readFileSync(dataPath, 'utf8') : '; data.asm \u2014 generated by Cooper.\n';
        if (!new RegExp(`^${base}_map:`, 'm').test(existing)) {
            fs.writeFileSync(dataPath, existing.replace(/\s*$/, '\n\n') + mapDataAsmSection(base, tmjRel, tsRel));
        }
    } catch (e) {
        fail(`could not create the tilemap: ${(e as Error).message}`);
        return;
    }
    const snippet = mapSnippet(base, sizePick.w);
    await vscode.env.clipboard.writeText(snippet);
    const doc = await vscode.workspace.openTextDocument({ language: 'c', content: snippet });
    await vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside);
    flash(`created ${tmjRel} \u2014 Makefile + data.asm wired; the mapLoad C is on your clipboard`);
    const open = await vscode.window.showInformationMessage(
        `Cooper: ${tmjRel} is ready \u2014 paint it in Tiled (tiles, collision via the 'attribute' property, spawns on the Entities layer), then Build.`,
        'Open in Tiled', 'Get Tiled');
    if (open === 'Open in Tiled') {
        const r = spawn('tiled', [path.join(projectDir, tmjRel)], { detached: true, stdio: 'ignore' });
        r.on('error', () => void vscode.env.openExternal(vscode.Uri.parse('https://www.mapeditor.org/')));
        r.unref();
    } else if (open === 'Get Tiled') {
        void vscode.env.openExternal(vscode.Uri.parse('https://www.mapeditor.org/'));
    }
}
